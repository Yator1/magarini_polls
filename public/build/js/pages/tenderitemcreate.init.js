...
document.addEventListener("DOMContentLoaded", function () {
    // Add event listener to a parent element (replace "parentElement" with the actual parent ID or class)
    document.getElementById("newlink").addEventListener("change", function (event) {
        // Check if the changed element is an item category select
        if (event.target && event.target.id && event.target.id.startsWith("item_category_id-")) {
            // Get the closest parent row
            var parentRow = event.target.closest(".product");

            // Find the corresponding item select based on the naming convention within the row
            var itemSelectId = event.target.id.replace("item_category_id", "item_id");
            var itemSelect = parentRow.querySelector("#" + itemSelectId);
...
function new_link() {
    ...
    tr1.innerHTML = `
        ...
        <td class="text-start">
            ...
            <select class="form-control is-invalid bg-light border-0" name="item_category_id" id="item_category_id-${count}">
                <option value="" selected disabled>Select</option>
                ${itemCategories.map(category => `<option value="${category.id}">${category.name}</option>`).join('')}
            </select>
            <span class="invalid-feedback" role="alert" id="item_category_id-error-${count}"></span>
        </td>
        <td class="text-start">
            <label for="item_id" class="form-label">Select Item <span class="text-danger">*</span></label>
            <select class="form-control is-invalid bg-light border-0" name="item_id" id="item_id-${count}">
                <option value="" selected disabled>Select</option>
            </select>
            <span class="invalid-feedback" role="alert" id="item_id-error-${count}"></span>
        </td>
        <td class="text-start">
            <label for="productDetails-${count}" class="form-label">Select hospital <span class="text-danger">*</span></label>
